...
$sorgu = mysqli_query($baglanti, "SELECT *, CONCAT(yazaradi,' ', yazarsoyadi) AS AdSoyad FROM yazarveri");
...
<form  action="kitap_eklendi.php" method="post">
    ...
    Yazar Adı:    <select style="position:relative;left:95px" name="KitapISBN" id="KitapISBN">
     <option value="Seçim Yapılmadı">Seç</option>
    <?php while($satir = mysqli_fetch_assoc($sorgu)){?>
    <option value="<?php echo $satir["YazarKayitNo"];  ?>"><?php echo $satir["AdSoyad"];  ?></option>
  <?php  }?>
</select><br><br>
    
    Yayınevi Bilgisi:    <select style="position:relative;left:57px" name="YayieviNo" id="YayineviNo">
  ...
  <?php  }?>
    ...
    Kitabın Dili: <input  style="position:relative;left:95px"type="text" name="KitapDili"><br><br>
    <input  style="text-align:center;background-color:#212121; color: white" type="submit" value="Kitap Ekleme">
</form>
<!-- KODLAR -->   
</main>
